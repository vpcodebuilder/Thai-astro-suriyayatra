...
แบ่งเป็น:
    ...
    2. ดิถีร้าย — อัคนิโรธ / มหาสูญ / พิฆาต / ทรธึก / ทักทิน / ยมขันธ์
    3. ดิถีพรหมประสิทธิ์ — สากล (ไม่ขึ้นกับวาร)
    4. ฤกษ์ปัญจมหาเศรษฐี — มงคลทรัพย์เฉพาะ 5 วาร (ศุกร์/พุธ/อังคาร/เสาร์/พฤหัสบดี)
...
หมายเหตุ:
    ...
    - ฤกษ์มหานิยม จัดเป็น "นักษัตรเทวีฤกษ์" ไม่ใช่ดิถี — ดู nakshatra.py:เทวีฤกษ์

อ้างอิง:
    - palungjit.org/ฤกษ์อมฤตโชคตามตำราฤกษ์พรหมประสิทธิ์
    - mahamodo.com/tamnai/good_time_other_isan.aspx
    - astroneemo.net — ตารางดิถีร้าย (ทักทิน/ยมขันธ์/ทรทึก/อัคนิโรจน์)
        * "ดิถีวันทักทิน": อา-1, จ-4, อ-6, พุธ-9, พฤ-5, ศ-3, ส-7
          (โทษ: ตำหนิ ทักท้วง ครหา)
        * "ดิถีวันยมขันธ์": อา-12, จ-11, อ-7, พุธ-3, พฤ-6, ศ-8, ส-9
          (โทษ: เพลิงไฟ ไฟไหม้ เจ็บป่วย)
    - theluckyname.com/ดิถีอัคนิโรธ
    - 10luc.com (ดิถีมงคล 5 ประการ)
    - horoscope.trueid.net (ฤกษ์ปัญจมหาเศรษฐี — นันทะ/ภัทระ/ชัย/มิตร/ปุรณะ)
"""
...
# ดิถีทักทิน (ทัคธทิน) — วันร้าย ถูกตำหนิ ทักท้วง ครหา
# ที่มา: astroneemo.net "ดิถีวันทักทิน"
#   "อาทิตย์วารหนึ่งแท้ ทักทิน จันทร์สี่ภุมมินทร์ หกถ้วน
#    เก้าพุธห้าครูผิน พัวโทษ สามศุกร์เสาร์เจ็ดล้วน เร่งร้ายแรงทัณฑ์"
DITHI_THAKTHIN = {
    1: [1],     # อาทิตย์ — ขึ้น/แรม 1 ค่ำ
    2: [4],     # จันทร์ — ขึ้น/แรม 4 ค่ำ
    3: [6],     # อังคาร — ขึ้น/แรม 6 ค่ำ
    4: [9],     # พุธ — ขึ้น/แรม 9 ค่ำ
    5: [5],     # พฤหัสบดี — ขึ้น/แรม 5 ค่ำ
    6: [3],     # ศุกร์ — ขึ้น/แรม 3 ค่ำ
    7: [7],     # เสาร์ — ขึ้น/แรม 7 ค่ำ
}

# ดิถียมขันธ์ — วันร้ายเสมือนอยู่ในเงื้อมยม โทษเพลิงไฟ ไฟไหม้ เจ็บป่วย
# ที่มา: astroneemo.net "ดิถีวันยมขันธ์"
#   "สิบสองอาทิตย์ย้อม ยมขันธ์ สิบเอ็ดวารวันจันทร์ จัดไว้
#    ภุมมสัตตะวัน พุธะสามนา ครูหกศุกร์แปดไซร้ อีกเก้าเสารี"
DITHI_YAMKHAN = {
    1: [12],    # อาทิตย์ — ขึ้น/แรม 12 ค่ำ
    2: [11],    # จันทร์ — ขึ้น/แรม 11 ค่ำ
    3: [7],     # อังคาร — ขึ้น/แรม 7 ค่ำ
    4: [3],     # พุธ — ขึ้น/แรม 3 ค่ำ
    5: [6],     # พฤหัสบดี — ขึ้น/แรม 6 ค่ำ
    6: [8],     # ศุกร์ — ขึ้น/แรม 8 ค่ำ
    7: [9],     # เสาร์ — ขึ้น/แรม 9 ค่ำ
}

# ฤกษ์ปัญจมหาเศรษฐี (ดิถีเศรษฐี 5) — มงคลทรัพย์สิน เปิดกิจการ บริหารเงิน
# ที่มา: trueid.net "ฤกษ์เศรษฐี ทั้ง 5 ปี 2566 หรือ ฤกษ์ปัญจมหาเศรษฐี"
#   5 ฤกษ์ ตั้งชื่อตามอัครมหาเศรษฐีสมัยพุทธกาล:
#     นันทมหาเศรษฐี — ศุกร์: 1, 6, 11
#     ภัทรมหาเศรษฐี — พุธ:    2, 7, 12
#     ชัยมหาเศรษฐี  — อังคาร: 3, 8, 13
#     มิตรมหาเศรษฐี — เสาร์:  4, 9, 14
#     ปุรณมหาเศรษฐี — พฤหัส:  5, 10, 15
# ใช้ได้ทั้งข้างขึ้นและข้างแรม (เลขเหมือนกัน)
DITHI_PANJA_SETTHI = {
    1: [],              # อาทิตย์ — ไม่มี
    2: [],              # จันทร์ — ไม่มี
    3: [3, 8, 13],      # อังคาร — ชัยมหาเศรษฐี
    4: [2, 7, 12],      # พุธ — ภัทรมหาเศรษฐี
    5: [5, 10, 15],     # พฤหัสบดี — ปุรณมหาเศรษฐี
    6: [1, 6, 11],      # ศุกร์ — นันทมหาเศรษฐี
    7: [4, 9, 14],      # เสาร์ — มิตรมหาเศรษฐี
}
...
DITHI_INFO = {
    ...
    "ทักทิน": DithiClassification(
        name="ดิถีทักทิน",
        is_auspicious=False, severity=2,
        short_desc="วันถูกตำหนิ — ครหา ทักท้วง ห้ามมงคล",
        long_desc=(
            "ดิถีทักทิน (บางตำราเรียก \"ทัคธทิน\") เป็นวันที่ตำราโบราณ"
            "ห้ามทำการมงคล เพราะถือว่าจะถูกตำหนิ ติเตียน ทักท้วง "
            "และครหานินทาจากผู้อื่น ทำสิ่งใดมักไม่เป็นที่ยอมรับ "
            "(โคลงโบราณ: อาทิตย์วารหนึ่งแท้ ทักทิน / จันทร์สี่ภุมมินทร์หกถ้วน / "
            "เก้าพุธห้าครูผิน / สามศุกร์เสาร์เจ็ดล้วน เร่งร้ายแรงทัณฑ์)"
        ),
        relevant_categories=("universal_bad",),
        suitable_for="ห้ามมงคลทุกประเภท",
    ),
    "ยมขันธ์": DithiClassification(
        name="ดิถียมขันธ์",
        is_auspicious=False, severity=3,
        short_desc="วันอันตรายเพลิงไฟ — ไฟไหม้ เจ็บป่วย ห้ามมงคล",
        long_desc=(
            "ดิถียมขันธ์ เปรียบเสมือนตกอยู่ในเงื้อมพระยม เป็นวันที่ตำรา"
            "โบราณห้ามทำการมงคลโดยเด็ดขาด เพราะให้โทษทางอันตรายจาก"
            "เพลิงไฟ ไฟไหม้ การเจ็บไข้ได้ป่วย โดยเฉพาะไม่ควรขึ้นบ้านใหม่ "
            "ตั้งศาล หรือพิธีกรรมที่เกี่ยวกับไฟ "
            "(โคลงโบราณ: สิบสองอาทิตย์ย้อมยมขันธ์ / สิบเอ็ดวารวันจันทร์จัดไว้ / "
            "ภุมมสัตตะวันพุธะสามนา / ครูหกศุกร์แปดไซร้ อีกเก้าเสารี)"
        ),
        relevant_categories=("universal_bad",),
        suitable_for="ห้ามมงคลทุกประเภท (โดยเฉพาะที่เกี่ยวกับไฟ/บ้าน)",
    ),
    "เศรษฐี": DithiClassification(
        name="ฤกษ์ปัญจมหาเศรษฐี",
        is_auspicious=True, severity=3,
        short_desc="วันมงคลทรัพย์ — เปิดกิจการ บริหารเงิน",
        long_desc=(
            "ฤกษ์ปัญจมหาเศรษฐี (ดิถีเศรษฐี 5) เป็นวันมงคลที่ตั้งชื่อตาม"
            "อัครมหาเศรษฐี 5 ท่านในสมัยพุทธกาล (นันทะ ภัทระ ชัย มิตร ปุรณะ) "
            "เป็นฤกษ์เปิดประตูทรัพย์ เหมาะอย่างยิ่งกับการเปิดกิจการ "
            "เปิดร้านค้า เซ็นสัญญาทางการเงิน ลงทุน เปิดบัญชี "
            "ตรงกับ ขึ้น/แรม 1,6,11 วันศุกร์ — 2,7,12 วันพุธ — "
            "3,8,13 วันอังคาร — 4,9,14 วันเสาร์ — 5,10,15 วันพฤหัสบดี"
        ),
        relevant_categories=("business", "ceremony"),
        suitable_for="เหมาะเปิดกิจการ/บริหารทรัพย์/เซ็นสัญญาการเงิน",
    ),
...
def classify_dithi(
    ...
    if day_in_phase in DITHI_PANJA_SETTHI.get(wan, []):
        matches.append(DITHI_INFO["เศรษฐี"])
    ...
    if day_in_phase in DITHI_THAKTHIN.get(wan, []):
        matches.append(DITHI_INFO["ทักทิน"])
    if day_in_phase in DITHI_YAMKHAN.get(wan, []):
        matches.append(DITHI_INFO["ยมขันธ์"])
